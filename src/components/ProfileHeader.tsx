...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ 
  ...
  return (
    <div className={`px-4 py-3 ${theme === 'dark' ? 'bg-[#121212]' : 'bg-white'}`}>
      ...
      <div className="flex mb-5 items-center">
        ...
        <div className="flex justify-between flex-grow text-center">
          ...
          <div>
            <div className={`font-semibold ${textColor}`}>{stats.followers >= 1000000 
              ? `${(stats.followers / 1000000).toFixed(1)}M` 
              : stats.followers >= 1000 
                ? `${(stats.followers / 1000).toFixed(1)}K` 
                : stats.followers.toLocaleString()}</div>
      ...
      {/* Highlights */}
      {showStories && (
        <div className="flex space-x-5 overflow-x-auto py-3 mb-2 scrollbar-none">
          {defaultStoryImages.map((image, index) => (
